...
import { deviceMeta, type DeviceType } from '@/components/network/deviceRegistry'
import { DeviceNode, type DeviceNodeType } from '@/components/network/DeviceNode'
...
import { newNodeId, readModel, serializeModel, type NetModel, type NetNode } from '@/components/network/networkTopologyOps'
import {
  createVlan,
  dissolveVlan,
  newDraft,
...
function EditorInner({ data, onSave, onClose }: EditorProps) {
  ...
  const [nodes, setNodes, onNodesChange] = useNodesState<DeviceNodeType>(
    initial.nodes.map((n) => ({ id: n.id, type: 'device', position: { x: n.x, y: n.y }, data: n })),
  )
  const [edges, setEdges, onEdgesChange] = useEdgesState<Edge>(
    initial.connections.map((c, i) => ({ id: `e-${c.from}-${c.to}-${i}`, source: c.from, target: c.to })),
  )
  ...
  const selected = singleId ? (nodes.find((n) => n.id === singleId)?.data ?? null) : null

  const onConnect = useCallback(
    (c: Connection) => {
      if (c.source && c.target && c.source !== c.target) setEdges((eds) => addEdge(c, eds))
    },
    [setEdges],
  )
  ...
  const handleNodesDelete = useCallback((deleted: Node[]) => {
    setVlans((vs) => deleted.reduce((acc, n) => pruneNodeFromVlans(acc, n.id), vs))
  }, [])

  const addDevice = useCallback(
    (type: DeviceType) => {
      const center = screenToFlowPosition({ x: window.innerWidth / 2 - 180, y: window.innerHeight / 2 })
      const id = newNodeId()
      setNodes((nds) => {
        const step = nds.length % 6 // stagger — לא לערום רכיבים חדשים
        const pos = { x: center.x + step * 40, y: center.y + step * 34 }
        const node: NetNode = { id, type, label: deviceMeta(type).label, ip: '', subnet: '', port: '', x: pos.x, y: pos.y }
        return [...nds.map((n) => ({ ...n, selected: false })), { id, type: 'device', position: pos, data: node, selected: true }]
      })
      setSelectedIds([id])
    },
  ...
  const patchSelected = useCallback(
    (patch: Partial<NetNode>) => {
      if (!singleId) return
      setNodes((nds) => nds.map((n) => (n.id === singleId ? { ...n, data: { ...n.data, ...patch } } : n)))
    },
    [singleId, setNodes],
  )
  ...
  const deleteSelected = useCallback(() => {
    ...
    setEdges((eds) => eds.filter((e) => e.source !== singleId && e.target !== singleId))
  ...
  const openCreateVlan = useCallback(() => {
    setDialog({ mode: 'create', editId: null, draft: newDraft(vlans), members: selectedIds })
  }, [vlans, selectedIds])
  ...
  const openEditVlan = useCallback(
    (id: string) => {
      ...
      if (!v) return
      setDialog({ mode: 'edit', editId: id, draft: { name: v.name, vlan_id: v.vlan_id, color: v.color }, members: v.member_node_ids })
    },
    [vlans],
  )
  ...
  const saveVlan = useCallback(() => {
    if (!dialog) return
    if (dialog.mode === 'create') setVlans((vs) => createVlan(vs, dialog.draft, dialog.members))
    else if (dialog.editId) setVlans((vs) => updateVlan(vs, dialog.editId as string, dialog.draft))
    setDialog(null)
    clearSelection()
  }, [dialog, clearSelection])

  const dissolveFromDialog = useCallback(() => {
    if (dialog?.editId) setVlans((vs) => dissolveVlan(vs, dialog.editId as string))
    setDialog(null)
  }, [dialog])
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col" style={{ background: STUDIO.stageBg }}>
      {/* top bar */}
      <div
      ...
      >
        <div className="min-w-0">
          <div className="text-[15px] font-semibold" style={{ color: STUDIO.textPrimary }}>
        ...
        <div className="relative min-w-0 flex-1">
          <VlanProvider value={vlans}>
            ...
            >
              ...
              <MiniMap pannable zoomable maskColor="rgba(16,21,28,.7)" style={{ background: '#141920' }} nodeColor="#2EB4FF" />
              <VlanRegions vlans={vlans} nodes={nodes} onEditVlan={openEditVlan} />
            </ReactFlow>
          </VlanProvider>
          ...
          {multiCount >= 2 && (
            <div
              ...
              style={{ background: 'rgba(32,38,47,.97)', border: '1px solid rgba(124,203,255,.35)' }}
            >
              <span className="inline-flex items-center gap-2 text-[12.5px] font-semibold" style={{ color: '#CFE0F0' }}>
      ...
      {dialog && (
        <VlanDialog
          ...
          onChange={(patch) => setDialog((d) => (d ? { ...d, draft: { ...d.draft, ...patch } } : d))}
          onSave={saveVlan}
