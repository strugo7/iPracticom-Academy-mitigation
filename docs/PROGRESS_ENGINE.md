...
## 14. שתי הגדרות של "אחוז התקדמות" — נפתח בשלב 3.1 (2026-07-10)

**לא נעים, אך מכוון:** מאז שלב 3.1 (learning catalog + track details) קיימות
בקוד **שתי הגדרות שונות** ל"כמה מהמסלול השלמתי", ושתיהן נכונות — הן פשוט
עונות על שאלות שונות. עתיד לצוץ שוב כשה-Dashboard האמיתי (עדיין
`PagePlaceholder`) יציג `avg_progress` לצד מסכי הלמידה שכבר בנויים.

| הגדרה | מקור | מונה | מכנה | עבור tallevi / מסלול `689a24dc...` |
|---|---|---|---|---|
| `avg_progress` (המנוע, §4-5 למעלה) | `recalculateUserStats` | `lessons_completed` — **כל השלמה ייחודית אי-פעם**, כולל שיעורים שנמחקו מאז מהקטלוג (§13.2: 17 אירועים כאלה בדאטה) | `total_lessons_in_track` — שיעורי published בקטלוג העדכני | **62%** (24/39) |
| `TrackDetailsViewModel.percent` (`trackDetailsService.ts`, feature הלמידה) | `assembleTrackDetails` | שיעורים שהושלמו **וגם עדיין קיימים** בעץ התוכן הנוכחי בלבד — לא ניתן להציג שורה לתוכן שנמחק | אותו מכנה, נגזר מאותה שרשרת track→module→topic→lesson | **38%** (15/39) |

שני המספרים "נכונים" — `avg_progress` הוא "כמה השגתי אי-פעם" (מדיניות מכוונת
של §1: "history is kept"), ו-`percent` של trackDetailsService הוא "כמה
מהתוכן שאני *רואה עכשיו* השלמתי". `TracksCatalog`/`TrackDetails` (feature
הלמידה) מציגים את השני בכוונה — כדי ששני המסכים האלה יסכימו זה עם זה
(תוקן ב-Task 17 של שלב 3.1: לפני התיקון הקטלוג הציג `avg_progress` והדף
הפנימי הציג את מספרו-שלו, ונוצרה אי-התאמה גלויה של 62% מול 38% לאותו מסלול
באותו רגע).

**פתוח, לא מוכרע:** כשה-Dashboard האמיתי ייבנה ויציג `avg_progress` — הוא
יראה 62% בזמן שמסכי הלמידה מראים 38%, אותה אי-התאמה שהתוקנה כאן בין שני
מסכים אחרים. שתי אפשרויות לשלב הבא: (א) ליישר את המנוע כך ש-`lessons_completed`
יספור רק שיעורים שעדיין בקטלוג (משנה התנהגות שכבר אושרה ונבדקה ב-§1-§13 —
טעון re-validation מול הדאטה האמיתי), או (ב) לתעד את שני המספרים כלגיטימיים
ולתייג בבירור באיזה משטח כל אחד מופיע. לא הוכרע — דורש בחירה מכוונת לפני
שה-Dashboard נבנה, לא ברירת-מחדל בשוגג.
